...
These settings are configured with environment variables, read from a `.env` file in the
...
The Orchard app itself has some configuration options, separate from the services it connects to below.
...
Each service below is optional and independent of the rest. Connect whichever ones you
want. Each unlocks its matching features in the application. That might be a full
