...
```bash
git clone https://github.com/cashubtc/orchard.git && cd orchard
git fetch --tags
git checkout v1.9.0
```
...
* Change to the `orchard` user, check out the new release, and rebuild (replace
  `v1.9.0` with the release you want)
...
<CardGrid>
  <LinkCard
    title="Using Orchard"
    href="/orchard/"
    description="The dashboard, monitoring, backups, and configuration at a glance."
  />
  <LinkCard
